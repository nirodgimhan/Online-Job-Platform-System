...
// ==================== ROUTES THAT DO NOT USE :id (MUST COME FIRST) ====================
...
router.put('/notifications', auth, async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).json({ success: false, message: 'User not found' });
        res.json({ success: true, message: 'Notification preferences updated', notifications: user.notificationPreferences });
    } catch (error) {
...
router.put('/profile-picture', auth, async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Profile picture updated', profilePicture: profilePictureUrl, user });
    } catch (error) {
...
router.get('/stats/dashboard', auth, auth.authorize('admin'), async (req, res) => {
    try {
        ...
        const unverifiedUsers = await User.countDocuments({ isVerified: false, role: { $ne: 'admin' } });
        const recentUsers = await User.find().select('-password').sort({ createdAt: -1 }).limit(5);
        res.json({
            success: true,
            stats: { totalUsers, totalStudents, totalCompanies, totalAdmins, verifiedUsers, unverifiedUsers },
            recentUsers
        });
    } catch (error) {
...
router.get('/:id', auth, async (req, res) => {
    try {
        ...
        else if (user.role === 'company') profileData = await Company.findOne({ userId: user._id });
        res.json({ success: true, user: { ...user.toObject(), profile: profileData } });
    } catch (error) {
        console.error('❌ Error fetching user:', error);
        if (error.kind === 'ObjectId') return res.status(404).json({ success: false, message: 'User not found' });
        res.status(500).json({ success: false, message: 'Server Error: ' + error.message });
    }
});
...
router.delete('/:id', auth, auth.authorize('admin'), async (req, res) => {
    try {
        ...
        else if (user.role === 'company') await Company.findOneAndDelete({ userId: user._id });
        await user.deleteOne();
...
router.put('/:id/verify', auth, auth.authorize('admin'), async (req, res) => {
    try {
        ...
        const user = await User.findById(req.params.id);
        if (!user) return res.status(404).json({ success: false, message: 'User not found' });
        if (user.role === 'admin') return res.status(400).json({ success: false, message: 'Admin users are already verified' });
        user.isVerified = true;
        user.updatedAt = Date.now();
        await user.save();
        if (user.role === 'company') {
            await Company.findOneAndUpdate({ userId: user._id }, { $set: { verified: true } });
        }
        console.log('✅ User verified successfully');
        res.json({ success: true, user: { id: user._id, name: user.name, email: user.email, role: user.role, isVerified: user.isVerified }, message: 'User verified successfully' });
    } catch (error) {
...
router.put('/:id/toggle-status', auth, auth.authorize('admin'), async (req, res) => {
    try {
        ...
        res.json({ success: true, user: { id: user._id, isActive: user.isActive }, message: `User ${user.isActive ? 'activated' : 'deactivated'} successfully` });
    } catch (error) {
