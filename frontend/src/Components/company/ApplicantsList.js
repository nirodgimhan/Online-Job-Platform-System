...
const ApplicantsList = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        // Process applications to extract student data properly
        const processedApps = applicationsData.map(app => {
          // studentId may be an object (populated) or just the ID
          ...
          return {
            ...
            studentName: student.name || studentUser.name || 'Unknown Applicant',
            studentEmail: student.email || studentUser.email || 'No email',
            studentAvatar: student.profilePicture || studentUser.profilePicture,
  ...
  const handleDownloadCV = (cv) => {
    ...
    if (filePath) {
      window.open(`http://localhost:5000${filePath.startsWith('/') ? filePath : '/' + filePath}`, '_blank');
    } else {
  ...
  const getFileIcon = (filename) => {
    ...
    if (ext === 'pdf') return <FaFilePdf className="al-file-icon-pdf" />;
    if (ext === 'doc' || ext === 'docx') return <FaFileWord className="al-file-icon-word" />;
    return <FaFileAlt />;
  };
  ...
  const getStatusBadge = (status) => {
    ...
    const statusMap = {
      'pending': { class: 'al-status-pending', icon: <FaClock />, label: 'Pending' },
      'reviewed': { class: 'al-status-reviewed', icon: <FaEye />, label: 'Reviewed' },
      'shortlisted': { class: 'al-status-shortlisted', icon: <FaStar />, label: 'Shortlisted' },
      'interview': { class: 'al-status-interview', icon: <FaCalendarAlt />, label: 'Interview' },
      'accepted': { class: 'al-status-accepted', icon: <FaCheckCircle />, label: 'Accepted' },
      'rejected': { class: 'al-status-rejected', icon: <FaTimesCircle />, label: 'Rejected' }
    };
    
    const badge = statusMap[statusLower] || statusMap.pending;
    
    return (
      <span className={`al-status-badge ${badge.class}`}>
        {badge.icon}
        {badge.label}
      </span>
    );
  ...
  if (error) {
    return (
      <div className="al-error-container">
        <div className="al-error-card">
          <FaExclamationTriangle className="al-error-icon" />
          <h3>Error Loading Applications</h3>
          <p>{error}</p>
          <div className="al-error-actions">
            <button className="al-btn al-btn-primary" onClick={handleRetry}>
              <FaRedo /> Try Again
            </button>
            <button className="al-btn al-btn-outline-secondary" onClick={handleGoToLogin}>
              Go to Login
            </button>
          </div>
        </div>
  ...
  return (
    <div className="al-applicants-list">
      <div className="al-container">
        {/* Header */}
        <div className="al-page-header">
          <div className="al-header-left">
            <div className="al-header-icon-wrapper">
              <FaUsers className="al-header-icon" />
            </div>
            <div>
              <h1>Job Applications</h1>
              <p className="al-header-subtitle">Review and manage all your job applications</p>
            </div>
          </div>
          <div className="al-header-actions">
            <Link to="/company/interviews" className="al-btn al-btn-outline-primary">
              <FaCalendarAlt /> Manage Interviews
            </Link>
            <button className="al-refresh-btn" onClick={handleRefresh} disabled={refreshing}>
              <FaSyncAlt className={refreshing ? 'al-spin' : ''} />
        ...
        {filteredApplications.length > 0 && (
          <div className="al-stats-grid">
            <div className="al-stat-card al-stat-total">
              <div className="al-stat-icon"><FaUsers /></div>
              <div className="al-stat-info">
                <span className="al-stat-value">{stats.total}</span>
                <span className="al-stat-label">Total</span>
              </div>
            </div>
            <div className="al-stat-card al-stat-pending">
              <div className="al-stat-icon"><FaClock /></div>
              <div className="al-stat-info">
                <span className="al-stat-value">{stats.pending}</span>
                <span className="al-stat-label">Pending</span>
              </div>
            </div>
            <div className="al-stat-card al-stat-review">
              <div className="al-stat-icon"><FaEye /></div>
              <div className="al-stat-info">
                <span className="al-stat-value">{stats.reviewed}</span>
                <span className="al-stat-label">Reviewed</span>
              </div>
            </div>
            <div className="al-stat-card al-stat-interview">
              <div className="al-stat-icon"><FaCalendarAlt /></div>
              <div className="al-stat-info">
                <span className="al-stat-value">{stats.interviewed}</span>
                <span className="al-stat-label">Interview</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="al-filters-card">
          <div className="al-filters-content">
            <div className="al-search-wrapper">
              <div className="al-search-input-group">
            ...
            <div className="al-filters-row">
              ...
              <div className="al-filter-group">
                ...
                <div className="al-sort-wrapper">
                  ...
                  <button className="al-sort-btn" onClick={() => setFilters(prev => ({
                    ...prev,
              ...
              <button className="al-clear-filters" onClick={clearFilters}>
                <FaTimes /> Clear
              </button>
            </div>
        ...
        {applications.length === 0 ? (
          ...
          <div className="al-empty-state">
            <div className="al-empty-icon-wrapper">
              <FaSearch className="al-empty-icon" />
            </div>
            <h3>No Matching Applications</h3>
            <p>No applications match your search criteria.</p>
            <button className="al-btn al-btn-outline-primary" onClick={clearFilters}>
              Clear Filters
            </button>
          </div>
        ) : (
          <div className="al-applications-grid">
            {filteredApplications.map((app) => (
              <div key={app._id} className="al-application-card">
                <div className="al-card-header">
                  <div className="al-applicant-info">
                    <div className="al-applicant-avatar">
                      <FaUserCircle />
                    </div>
                    <div>
                      <h3>{app.studentName || 'Unknown Applicant'}</h3>
                      <p className="al-applicant-email">
                ...
                {app.coverLetter && (
                  <div className="al-cover-letter-preview">
                    <FaFileAlt />
                ...
                <div className="al-card-actions">
                  <button className="al-btn al-btn-outline-primary" onClick={() => handleViewDetails(app)}>
                    <FaEye /> View Details
                  </button>
                  <button className="al-btn al-btn-primary" onClick={() => handleScheduleInterview(app)}>
                    <FaCalendarPlus /> Schedule Interview
                  </button>
                  <button className="al-btn al-btn-outline-secondary" onClick={() => handleViewCV(app.studentId)} disabled={cvLoading}>
                    {cvLoading ? <FaSpinner className="al-spin" /> : <FaFileAlt />} CV
                  </button>
                </div>
              </div>
            ))}
      ...
      {showDetailsModal && selectedApplication && (
        <div className="al-modal-overlay" onClick={() => setShowDetailsModal(false)}>
          <div className="al-modal al-modal-large" onClick={e => e.stopPropagation()}>
            <div className="al-modal-header">
              <FaUserCircle className="al-modal-icon" />
              <h3>Application Details</h3>
              <button className="al-modal-close" onClick={() => setShowDetailsModal(false)}>
                <FaTimes />
              </button>
            </div>
            <div className="al-modal-body">
              <div className="al-details-grid">
                ...
                {selectedApplication.coverLetter && (
                  <div className="al-detail-row al-full-width">
                    <strong>Cover Letter:</strong>
                    <div className="al-cover-letter-full">
                      {selectedApplication.coverLetter}
                    </div>
                  </div>
                )}
            ...
            <div className="al-modal-footer">
              <select
                className="al-form-control al-status-select"
              ...
              <button className="al-btn al-btn-secondary" onClick={() => setShowDetailsModal(false)}>
                Close
              </button>
            </div>
          </div>
      ...
      {showInterviewModal && selectedApplication && (
        <div className="al-modal-overlay" onClick={() => setShowInterviewModal(false)}>
          <div className="al-modal al-modal-large" onClick={e => e.stopPropagation()}>
            <div className="al-modal-header al-modal-header-success">
              <FaCalendarPlus className="al-modal-icon" />
              <h3>Schedule Interview</h3>
              <button className="al-modal-close" onClick={() => setShowInterviewModal(false)}>
                <FaTimes />
              </button>
            </div>
            <div className="al-modal-body">
              <div className="al-interview-preview">
                <h4>{selectedApplication.studentName || 'Applicant'}</h4>
                <p>{selectedApplication.jobId?.title}</p>
              </div>

              <div className="al-form-group">
                <label>Date & Time <span className="al-required">*</span></label>
                <input
                  type="datetime-local"
              ...
              {interviewData.mode === 'Online' && (
                <div className="al-form-group">
                  <label>Meeting Link <span className="al-required">*</span></label>
                  <input
                    type="url"
              ...
              {interviewData.mode === 'In-person' && (
                <div className="al-form-group">
                  <label>Location <span className="al-required">*</span></label>
                  <input
                    type="text"
                    placeholder="Address"
            ...
            <div className="al-modal-footer">
              <button className="al-btn al-btn-secondary" onClick={() => setShowInterviewModal(false)}>
                Cancel
              </button>
              <button className="al-btn al-btn-primary" onClick={handleCreateInterview} disabled={interviewLoading}>
                {interviewLoading ? <><FaSpinner className="al-spin" /> Scheduling...</> : <><FaCalendarPlus /> Schedule Interview</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* CV Modal */}
      {showCVModal && selectedCVs.length > 0 && (
        <div className="al-modal-overlay" onClick={() => setShowCVModal(false)}>
          <div className="al-modal al-modal-large" onClick={e => e.stopPropagation()}>
            <div className="al-modal-header al-modal-header-info">
              <FaFileAlt className="al-modal-icon" />
              <h3>Applicant CV / Resume</h3>
              <button className="al-modal-close" onClick={() => setShowCVModal(false)}>
                <FaTimes />
              </button>
            </div>
            <div className="al-modal-body">
              <div className="al-cv-list">
                {selectedCVs.map((cv, index) => (
                  <div key={cv._id || index} className="al-cv-item">
                    <div className="al-cv-icon">
                      {getFileIcon(cv.filename || cv.fileName)}
                    </div>
                    <div className="al-cv-info">
                      <h4>{cv.title || cv.filename || cv.fileName || 'CV Document'}</h4>
                      <div className="al-cv-meta">
                        <span><FaCalendarAlt /> Uploaded: {formatDate(cv.uploadedAt || cv.createdAt)}</span>
                        <span><FaFileAlt /> {formatFileSize(cv.fileSize)}</span>
                        {cv.isPrimary && <span className="al-primary-badge">Primary CV</span>}
                      </div>
                    </div>
                    <div className="al-cv-actions">
                      <button className="al-btn-icon" onClick={() => handleDownloadCV(cv)} title="View CV">
                        <FaEye />
                      </button>
                    </div>
                  </div>
            ...
            <div className="al-modal-footer">
              <button className="al-btn al-btn-secondary" onClick={() => setShowCVModal(false)}>
                Close
              </button>
            </div>
          </div>
